...
- **`pr-style`**（development）→ `git merge --no-ff` 将整合分支并入目标分支；apply 后删除 integration 分支 + 清理临时 worktree（`git worktree remove`）。**保留 `.agent-team/evidence/worktrees.jsonl`**（ADR-0055：worktree 隔离证据，供事后审计，勿随 worktree 一起清掉）
